...
impl Connection {
    ...
    pub async fn get_finished_build(
        ...
        store_dir: &StoreDir,
        build_id: BuildID,
    ) -> anyhow::Result<Option<FinishedBuild>> {
        ...
        let mut raw_outputs = sqlx::query_as::<_, (String, Option<String>)>(
        ...
        if raw_outputs.is_empty() && status == BuildStatus::Success {
            // Rejected builds can still have successful step rows; only reuse outputs from accepted builds.
            raw_outputs = sqlx::query_as::<_, (String, Option<String>)>(
...
pub struct FinishedBuild {
    ...
    pub outputs: BTreeMap<OutputName, Option<StorePath>>,
}
...
mod tests {
    ...
    async fn get_finished_build_returns_none_while_unfinished() {
        ...
        assert!(
            conn.get_finished_build(&test_store_dir(), 1)
                .await
                .unwrap()
                .is_none()
        );
    ...
    async fn get_finished_build_uses_own_step_outputs() {
        ...
        let r = conn
            .get_finished_build(&test_store_dir(), 1)
            .await
            .unwrap()
            .unwrap();
        assert_eq!(r.status, BuildStatus::Success);
        assert_eq!(r.outputs, BTreeMap::from([(on("out"), Some(sp("result")))]),);
    }
    ...
    async fn get_finished_build_falls_back_when_own_steps_missing() {
        ...
        let r = conn
            .get_finished_build(&test_store_dir(), 2)
            .await
            .unwrap()
            .unwrap();
        assert_eq!(r.outputs, BTreeMap::from([(on("out"), Some(sp("result")))]),);
    ...
    async fn get_finished_build_fallback_picks_most_recent_successful() {
        ...
        let r = conn
            .get_finished_build(&test_store_dir(), 3)
            .await
            .unwrap()
            .unwrap();
        assert_eq!(
            r.outputs,
            BTreeMap::from([(on("out"), Some(sp("result-v2")))]),
        );
    }
