...
pub enum ListenSource {
    /// Bind a new Unix socket at this path.
    Path(PathBuf),
    /// Adopt a socket from systemd / foreman socket activation (`LISTEN_FDS`).
    Activated,
}
...
{
    pub fn new(handler: H, listen: ListenSource, store_dir: StoreDir) -> Self {
        Self {
            handler,
            listen,
            store_dir,
    ...
    pub async fn serve(&self) -> Result<(), std::io::Error> {
        let listener = match &self.listen {
            ...
            ListenSource::Activated => {
                let mut listenfd = listenfd::ListenFd::from_env();
                let std_listener = listenfd
                    .take_unix_listener(0)?
                    .expect("no Unix listener at LISTEN_FDS index 0");
                std_listener.set_nonblocking(true)?;
                let listener = UnixListener::from_std(std_listener)?;
                info!("nix daemon using socket activation");
                listener
            }
        };
