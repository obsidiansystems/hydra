...
sub clear_queue_non_current : Chained('admin') PathPart('clear-queue-non-current') Args(0) {
    ...
    my $builds = $c->model('DB::Builds')->search_rs(
        { id => { -in => \ "select id from Builds where id in ((select id from Builds where stopTime is null) except (select build from JobsetEvalMembers where eval in (select max(id) from JobsetEvals where hasNewBuilds = 1 group by jobset_id)))" }
        });
