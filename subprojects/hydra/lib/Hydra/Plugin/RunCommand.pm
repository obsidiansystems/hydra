...
sub isBuildEligibleForDynamicRunCommand {
    ...
    if ($build->get_column("job") =~ "^runCommandHook\..+") {
        ...
        # These are filesystem tests, so they need the full path.
        my $path = printStorePath(machineLocalStore()->storeDir, $out->path);
        if (-l $path) {
            $path = readlink($path);
        }
    ...
    if (areDynamicCommandsEnabled($config)) {
        if (isBuildEligibleForDynamicRunCommand($build)) {
            ...
            push(@commands, {
                ...
                command => printStorePath(machineLocalStore()->storeDir, $out->path)
            })
        }
    }
