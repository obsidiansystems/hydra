...
impl State {
    ...
    async fn create_step(
        ...
        // Output paths from the nix derivation. Covers all derivations except CAFloating
        let calculated_outputs = nix_utils::output_paths(&drv, self.store.store_dir());
        // Output paths from the hydra database. They should exist for all built derivations,
        // either CAFloating or not.
        let database_outputs = self.query_drv_outputs(&drv_path).await.unwrap_or_else(|e| {
            tracing::warn!("Could not query known outputs, continuing: {e}");
            BTreeMap::new()
        });
        // Combo of calculated and database, includes built CAFloating and both built/unbuilt
        // non-CAFloating
        let known_outputs = calculated_outputs
            .iter()
            .map(|(name, value)| {
                (
                    name.clone(),
                    value
                        .clone()
                        .or_else(|| database_outputs.get(name).cloned()),
                )
            })
            .collect::<BTreeMap<_, _>>();

        // TODO: query build traces from substituter

        // Known outputs that are *not* in the store
        let missing_local_outputs = self
            .store
            .query_missing_outputs(known_outputs.clone())
            .await;

        // Non-CAFloating outputs that exist in the nix store but need to be registered in the database
        // This is required when a CAFloating derivation depends on a non-CAFloating derivation.
        // CAFloating derivation resolution requires knowing the outputs of all dependencies,
        // and since the resolution happens in the DB, the outputs must also be registered there.
        {
            let unregistered_local_outputs = calculated_outputs
                .iter()
                .filter(|(name, path)| {
                    path.is_some()
                        && !missing_local_outputs.contains_key(&name)
                        && !database_outputs.contains_key(&name)
                })
        ...
        let missing_outputs = if let Some(ref remote_store) = remote_store {
            let mut missing_remote_outputs = remote_store
                .query_missing_remote_outputs(known_outputs.clone())
                .await;
            if !missing_remote_outputs.is_empty() && missing_local_outputs.is_empty() {
                ...
                if let Ok(log_file) = self.construct_log_file_path(&drv_path).await {
                    let missing_paths: Vec<nix_utils::StorePath> = missing_remote_outputs
                        .values()
                    ...
                    self.uploader
                        ...
                        .await;
                    missing_remote_outputs.clear();
                }
            }
            missing_remote_outputs
        } else {
    ...
    #[tracing::instrument(skip(self))]
    async fn query_drv_outputs(
        &self,
        drv_path: &nix_utils::StorePath,
    ) -> anyhow::Result<BTreeMap<nix_utils::OutputName, nix_utils::StorePath>> {
        let mut db = self.db.get().await?;
        let mut tx = db.begin_transaction().await?;
        tx.query_drv_outputs(&self.store.print_store_path(drv_path))
            .await?
            .iter()
